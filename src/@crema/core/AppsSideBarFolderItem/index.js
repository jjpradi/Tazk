...
const AppsSideBarFolderItem = ({item, path, type,setType,type_details,setType_details, alias, isactive, setIsactive}) => {

 
  const TypeChange = (type, name, id) =>{
    setType(type);
    setType_details(name);
    setIsactive(id);
  }
  return (
    <AppsSideBarFolderList
      ...
      onClick= {(e)=>{
        e.preventDefault();
        TypeChange(item.type, item.type_name, item.id)} }
      component={AppNavLink}
      activeClassName={isactive === item.id ? 'active' : ''}
    >
      <ListItemIcon
        sx={{
          minWidth: 10,
