...
const CompletedTask = ({ projectname,duedate,search,employee}) => {
  ...
  await dispatch(
    getProjectDetailsAction(val.project_id, async (response) => {
      ...
      const handleDelete = (event) => {
        event.stopPropagation();
        dispatch(tasksDeleteAction({ id: selectedTaskId, type: "completed" }, async () => {
          refetchTasks();
        }));
        setDeleteDialog(false);
      };
    ...
    const deletecompletedtask = getRoleAuthorization(user_rights,'DeleteTask')
          ...
          tasks?.map((t, index) => (
            ...
            <CardContent style={{ display: 'flex', alignItems: 'center', padding: '4px 8px' }}>
              ...
              <CardContent style={{ padding: '0 8px', display: 'flex', alignItems: 'center' }}>
                <Typography variant="body2" sx={{ whiteSpace: 'nowrap', pt:3,pb:2,overflow: 'hidden', textOverflow: 'ellipsis',fontFamily: 'poppins', fontSize: '13px', fontWeight: '400', color: 'rgba(0, 0, 0, 0.7)', }}>
                  {t?.project_name}
                </Typography>
              </CardContent>
              ...
              <CardContent style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', padding: '4px 8px 4px 8px', marginTop: 'auto' ,fontFamily: 'poppins', fontSize: '11px', fontWeight: '400', color: 'rgba(0, 0, 0, 0.7)'}}>
                <Typography variant="body2" sx={{ fontSize: '0.7rem', marginRight: '4px' }}>
                  {t?.task_id}
                </Typography>
                <Typography variant="body2" sx={{ display: 'flex', alignItems: 'center' ,fontFamily: 'poppins', fontSize: '13px', fontWeight: '400', color: 'rgba(0, 0, 0, 0.7)'}}>
