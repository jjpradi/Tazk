...
import { taskDetailsCountAction , getActivityAction, filterTaskDetailsAction,createTaskAction,getTaskByStatusAction,updateTaskAction,showTasklistAction, tasksDeleteAction, getProjectDetailsAction} from 'redux/actions/payrollDashboard_actions';
...
const IncompletedTask = ({projectname,duedate,search,employee}) => {
  ...
  await dispatch(
    getProjectDetailsAction(val.project_id, async (response) => {
      ...
      setselectedTaskForEdit({
        ...
        parent_issue_type: val?.parent_issue_type ?? val?.parentIssueType ?? '',
      ...
      const handleDelete = (event) => {
        event.stopPropagation();
        dispatch(tasksDeleteAction({ id: selectedTaskId, type: "incompleted" }, async () => {
          refetchTasks();
        }));
        setDeleteDialog(false);
      };
        ...
        if (actionName === 'copy') {
          ...
          if (selectedTaskForEdit.project_id) {
            ...
            apiCalls(
              ...
              dispatch(updateTaskAction( resolvedTaskId,{
                ...payload,
    ...
    const deleteincompletetask = getRoleAuthorization(user_rights,'DeleteTask')

    return (
      <>
        <Grid container direction="row" spacing={3} sx={{ overflowX: "auto" , paddingBottom: '16px',paddingLeft: '16px', paddingRight: '16px' , paddingTop: '16px' }}>
          ...
          tasks?.map((t, index) => (
            <Grid
              ...
              <div style={{ cursor: 'pointer', position: 'relative' }}
               ...
               <Card style={{ height: '150px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' ,  padding: '8px'}}>
                 {/* IconButton */}
            ...
            <CardContent style={{ display: 'flex', alignItems: 'center', padding: '4px 8px' }}>
                ...
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold', fontSize: '0.85rem', marginRight: '8px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',fontFamily: 'poppins'  }}>
                  {t.assignee_name || 'Unassigned'}
                </Typography>
              </CardContent>
              <CardContent style={{ padding: '0 8px', display: 'flex', alignItems: 'center' }}>
                <Typography variant="body2" sx={{ whiteSpace: 'nowrap', pt:3, pb:2,overflow: 'hidden', textOverflow: 'ellipsis',fontFamily: 'poppins', fontSize: '14px', fontWeight: '400', color: 'rgba(0, 0, 0, 0.7)' }}>
                  {t.project_name}
                </Typography>
              </CardContent>
              <CardContent style={{ padding: '0 8px 4px 8px', display: 'flex', alignItems: 'center' }}>
                <Typography variant="body2" sx={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' ,fontFamily: 'poppins', fontSize: '13px', fontWeight: '400', color: 'rgba(0, 0, 0, 0.7)', marginLeft: '4px', }}>
                  {t.task_name}
                </Typography>
              </CardContent>
              <CardContent style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', padding: '4px 8px 4px 8px', marginTop: 'auto' , fontFamily: 'poppins', fontSize: '14px', fontWeight: '400', color: 'rgba(0, 0, 0, 0.7)'}}>
                <Typography variant="body2" sx={{ fontSize: '0.7rem', marginRight: '4px' }}>
                  {t.task_id}
                </Typography>
                <Typography variant="body2" sx={{ display: 'flex', alignItems: 'center', fontFamily: 'poppins', fontSize: '14px', fontWeight: '400', color: 'rgba(0, 0, 0, 0.7)' }}>
