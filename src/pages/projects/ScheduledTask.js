...
import { Dialog, Grid,Tooltip ,DialogContent,MenuItem, Menu, DialogContentText,DialogTitle, IconButton , DialogActions,Button } from '@mui/material';
...
const ScheduledTask = ({ projectname,duedate,search,employee}) => {
  ...
  await dispatch(
    getProjectDetailsAction(val.project_id, async (response) => {
      ...
      const handleDelete = (event) => {
        event.stopPropagation();
        dispatch(tasksDeleteAction({ id: selectedTaskId, type: "scheduled" }, async () => {
          refetchTasks();
        }));
        setDeleteDialog(false);
      };
  ...
  const handleChangeEmployeeName =(val)=>{
    ...
    const deletescheduledtask = getRoleAuthorization(user_rights,'DeleteTask')
              ...
              <Grid
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Typography variant='body1' sx={{marginBottom: '8px', pt:3}}>
                        {t.project_name}
                      </Typography>
                    </CardContent>
                    <CardContent
