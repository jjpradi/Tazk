...
import {
  ...
  getSearchOverTimeReportAction,
  setSearchOverTimeReportAction,
...
import {
  getRelievedEmployeeDetailsAction,
  getSearchRelievedEmployeeDetails,
  setSearchRelievedEmployeeDetails,
...
const DeletedEmployeeDetails = (props) => {
  ...
  const {
    reportsReducer: {relievedEmpDetails},
  ...
  useEffect(() => {
    ...
    if (!flag) {
      ...
      payLoad = {
        type: 'deleted',
        fromDate: moment(previousDate.from, 'year', 'month', 'day').format(
          'YYYY-MM-DD',
        ),
        toDate: moment(previousDate.to, 'year', 'month', 'day').format(
          'YYYY-MM-DD',
        ),
        pageCount: page,
        numPerPage: pageSize,
  ...
  const handlePreviousMonthClick = (data) => {
    ...
    setFilterDate({...filterDate, from: startDate, to: endDate});
    let payLoad = {
      type: 'deleted',
      fromDate: moment(startDateObj, 'year', 'month', 'day').format(
        'YYYY-MM-DD',
      ),
      toDate: moment(endDateObj, 'year', 'month', 'day').format('YYYY-MM-DD'),
      pageCount: 0,
      numPerPage: pageSize,
      searchString: '',
      location: [],
  ...
  const dataWithId = finalData?.length
    ? finalData?.map((row, index) => ({...row, id: index}))
    : [];
  ...
  return (
    <>
      <Helmet>
        <meta charSet='utf-8' />
        <title> {titleURL} | Deleted Employee Reports </title>
      </Helmet>
      <DataGridTemp
        columns={columns}
        columnData={columns}
