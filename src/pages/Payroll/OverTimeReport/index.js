...
const OverTimeReport = (props) => {
    ...
    const { attendanceReducer: { overTimeReport }, 
    stockLocationReducer: { stocklocation },
     ShiftsReducer: { list_department }, rbacReducer: {menuAccess = []}} = useSelector((state) => state);
    ...
    console.log(overTimeReport.data, "overTimeReport data");
    console.log(overTimeReport.numRows, "overTimeReport numRows");

    console.log('asfsfe', monthYear)
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        setSearchOverTimeReportAction({ data: [], numRows: 0 });
        let payLoad
        if (!flag) {
            payLoad = {
        ...
        apiCalls(
            dispatch(getSearchOverTimeReportAction(payLoad))).finally(() =>
        { 
    ...
    const columns = [
        { field: 'employee_code', headerName: 'Emp Code', width: 200 },
        ...
        { field: 'employee_work_hours', headerName: 'Total Work Hours', width: 230 },
        { field: 'permission_hours', headerName: 'Permission Hours', width: 230, valueGetter: (params) => params?.row?.permission_hours ?? params?.value ?? '' },
        { field: 'extra_work_time', headerName: 'Over Time', width: 230 },
        { field: 'workingDays', headerName: 'No of Working Days', width: 230 },
    ];
    ...
    const ApplyButton = async (val) => {
        ...
        setPageCount(0)
        console.log(payLoad, "load");
        dispatch(getSearchOverTimeReportAction(payLoad))
    ...
    const requestSearch = (e) => {
        ...
        dispatch(setSearchOverTimeReportAction({ data: [], numRows: 0 }));
        let payLoad = {
            fromDate: moment(filterDate.from, 'year', 'month', 'day').format('YYYY-MM-DD'),
            toDate: moment(filterDate.to, 'year', 'month', 'day').format('YYYY-MM-DD'),
            pageCount: pageCount,
    ...
    const handlePageChange = async (page) => {
        ...
        // console.log(payLoad,"load");
        //dispatch(getOverTimeReportAction(payLoad))
    }
    ...
    const handlePageSizeChange = async (size) => {
        ...
        // console.log(payLoad,"load");
        //d/ispatch(getOverTimeReportAction(payLoad))
    };
    const originalData = overTimeReport?.data;
    const dataWithId = originalData?.length ? originalData?.map((row, index) => ({ ...row, id: index })) : []
    console.log(overTimeReport, 'ss');
    ...
    const reportExport = UserRightsAuthorization(menuAccess[selectedRole], 'reports__attendance__overtime_report', 'can_export')
    const handleExport = async () => {
        ...
        link.setAttribute("download", 'Over Time Report' + ".csv");
    ...
    return (
        <>
            <Helmet>
                <meta charSet='utf-8' />
                <title> {titleURL} | Over Time Reports </title>
            </Helmet>
            <Card
                sx={{
                    width: '100%',
                ...
                <Box sx={{ flex: 1, minHeight: 0, overflow: 'hidden' }}>
                    {isApiFinished ? (
                        <DataGrid
                            ...
                            rowCount={overTimeReport?.numRows || 0}
...
//     return {
//         overTimeReport: state.attendanceReducer.overTimeReport 
//     };
//   };
//   const mapDispatchToProps = (dispatch) => {
//     return {
//         getSearchOverTimeReportAction: (val,setModalTypeHandler,setLoaderStatusHandler) => {
//             return dispatch(getSearchOverTimeReportAction(val,setModalTypeHandler,setLoaderStatusHandler),
//             );
//           },
//         setSearchOverTimeReportAction: (val) => {
//             return dispatch(setSearchOverTimeReportAction(val),
//             );
//           },
//         getOverTimeReportAction: (data) => { 
//             return dispatch(getOverTimeReportAction(data));
//           },
//     };
//   };

//   export default connect(mapStateToProps,mapDispatchToProps)(OverTimeReport);
