...
const Index = () => {
  ...
  const {
    ...
    stockLocationReducer: { stocklocation }, UserCreationReducer: {departmentList},    ShiftsReducer: { list_department }, rbacReducer: {menuAccess = []}
...
console.log("pageSizepageSize",pageSize);
  ...
  const handlePageChange = async (page) => {
    let data = {
      employee_id: get_empbasecompany.map((d) => d.employee_id),
      location_id: monthYear.location[0] === '' ? 'null' : monthYear.location,
      department: monthYear.department[0] === ''  ? list_department.map((d) => d.department)  : monthYear.department,
      from: moment(monthYear.from).format('yyyy-MM-DD'),
      to: moment(monthYear.to).format('yyyy-MM-DD'),
    ...
    },
  {
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div style={commonCellStyle}>
          {
            params.row.end_type === 'NO_RESTRICTION' ? 'NONE'
            : params.row.end_type === 'ATTENDANCE CORRECTION' ? 'REGULARIZED'
            : params.row.end_type === 'FACE_ATTENDANCE' ? 'FACE'
            : params.row.end_type === 'MANUAL_ATTENDANCE' ? 'MANUAL'
            : params.row.end_type === 'MANUAL_CORRECTION' ? 'ADJUSTED'
            : params.row.end_type === 'DEVICE' ? 'DEVICE'
            : params.row.end_type
          }
        </div>
      ),
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div style={commonCellStyle}>
          {params.row.type === 'ATTENDANCE CORRECTION' ? formatTime12Hour(params.row.first_in_time) : '-'}
        </div>
      ),
    },
    {
      ...
      renderCell: (params) => (
        <div style={commonCellStyle}>
          {params.row.type === 'ATTENDANCE CORRECTION'  ? formatTime12Hour(params.row.last_out_time) : '-'}
        </div>
      ),
    },
    {
...
const handleFilterClear = () =>{
  ...
  const reportExport = UserRightsAuthorization(menuAccess[selectedRole], 'reports__attendance__attendance_view_report', 'can_export')

  return (
    <>
      <Helmet>
        <meta charSet='utf-8' />
        <title> {titleURL} | Attendance View Report</title>
      </Helmet>
      <Card
        sx={{
          width: '100%',
