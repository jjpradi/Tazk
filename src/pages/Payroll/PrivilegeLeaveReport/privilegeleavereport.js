...
import { getEmpbasecompanyAction, getSearchprivilegeReportAction, getSearchpunchReportAction, lateAndEarlyReportAction, privilegeleavereportAction, punchexceptionAction, setSearchPrivilegeleaveAction, setSearchPunchReportAction } from 'redux/actions/attendance_actions';
...
const PrivilegeLeaveReport = (props) => {
    ...
    const { attendanceReducer: { privilegeleavereport }, 
    ...
    useEffect(() => {
        ...
        if (flag) {
         ...
         apiCalls(
            dispatch(privilegeleavereportAction(payLoad))
         ).finally(() =>
        { 
          setIsApiFinished(true)
    ...
    const columns = [
        ...
        { field: 'carry_forwarded_leave_count', headerName: 'Carry Forward', width: 230 },
        { field: 'current_leave_count', headerName: 'Current Leave', width: 230 },
        { field: 'opening_carry_forward_leave_count', headerName: 'Opening Carry Forward', width: 250 },
    ];
    ...
    const requestSearch = (e) => {
        ...
        setSearchVal(val)
        dispatch(setSearchPrivilegeleaveAction({ data: [], numRows: 0 }));
        let payLoad = {
            fromDate: moment(filterDate.from, 'year', 'month', 'day').format('YYYY-MM-DD'),
            toDate: moment(filterDate.to, 'year', 'month', 'day').format('YYYY-MM-DD'),
            pageCount: pageCount,
        ...
        console.log(payLoad, "load");
        dispatch(getSearchprivilegeReportAction(payLoad,
            setModalTypeHandler,
        (loaderStatus) => {
    ...
    const originalData = privilegeleavereport?.data;
    const dataWithId = originalData?.length ? originalData?.map((row, index) => ({ ...row, id: index })) : []
    console.log(privilegeleavereport, 'ss');
    ...
    const reportExport = UserRightsAuthorization(menuAccess[selectedRole], 'reports__attendance__privilege_leave', 'can_export')
    const handleExport = async () => {
        ...
        link.setAttribute("download", 'Privilege Leave' + ".csv");
    ...
    return (
        <>
            <Helmet>
                <meta charSet='utf-8' />
                <title> {titleURL} | Privilege Leave</title>
            </Helmet>
            <Card
                sx={{
                    width: '100%',
                ...
                <Box sx={{ flex: 1, minHeight: 0, overflow: 'hidden' }}>
                    {isApiFinished ? (
                        <DataGrid
                            ...
                            rowCount={privilegeleavereport?.numRows || 0}
...
export default PrivilegeLeaveReport;