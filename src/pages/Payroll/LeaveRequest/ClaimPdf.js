...
export default function ClaimPdf(props) {
  ...
  const {open, handleClose, claimDownloadData } = props;
  ...
  claimDownloadData.companyLogo = companyLogo;
  ...
  const handleDownload = () => {
    ...
    html2canvas(input, { useCORS: true }).then((canvas) => {
      ...
      pdf.save(`Claims_${claimDownloadData.full_name}.pdf`);
    }).catch(error => {
  ...
  useEffect(() => {
    ...
    if(slipRef.current){
      slipRef.current.innerHTML = ClaimTemp(claimDownloadData)
    }
    return () => {
      if(slipRef.current){
...
ClaimPdf.propTypes = {
  ...
  claimDownloadData: PropTypes.object,
};
