...
import { dayBookReportAction, setStockSummary, StockGroupSummaryAction } from 'redux/actions/sales_actions'
...
const StockGroupSummary = () => {
    ...
    const requestSearch = async(e)=>{
       ...
       await dispatch(setStockSummary(payload,setModalTypeHandler, setLoaderStatusHandler))
    ...
    useEffect(()=>{
        ...
        dispatch(StockGroupSummaryAction(payload))
    },[]) 
    ...
    const filterSubmit = async()=>{
      ...
      await dispatch(StockGroupSummaryAction(payload))
      setFilter(false)
    }
  ...
  const reportExport = UserRightsAuthorization(menuAccess[selectedRole], 'reports__inventory__stock_group_summary', 'can_export')
  return (
      <>
          <Helmet>
              <meta charSet='utf-8'/>
              <title>{titleURL} | Stock Group Summary Report</title>
          </Helmet>
          <DataGridTemp
              title={<Typography variant='h6' align='left' style={{ paddingTop: '10px', paddingBottom: '10px' }}>
                    {/* <Link href='/report' underline="hover">Home</Link> / Stock Group Summary Report */}
...
export default StockGroupSummary