...
import { TextField, InputAdornment, List, ListItem, ListItemText, Chip, Typography, Link, IconButton, Dialog, Box, Tabs, Tab, TablePagination } from '@mui/material';
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      Schedulecolumns  :  [
        ...
        { name: "Order Date", key: "sale_time" },
        { name: "Order Number", key: "so_number" },
        { name: "Invoice Number", key: "invoice_number" },
        { name: "Customer Name", key: "company_name" },
        { name: "Product", key: "product_name" },
        { name: "Category", key: "product_category" },
        { name: "Brand", key: "product_brand" },
        { name: "Serial Number", key: "serial_number" },
        { name: "HSN code", key: "hsn_code" },
        { name: "Purchase Price", key: "cost_price" },
        { name: "Sale Price", key: "unit_price" },
        { name: "Margin", key: "Margin" },
        { name: "Quantity", key: "ordered_qty" },
        { name: "Tax", key: "sale_tax_amount" },
        { name: "Total", key: "total" },
        { name: "Tax Category", key: "tax_category" }
      ],
      tabValue: 'lotWise'
    };
    this.columns = [
                ...
                {
                  title: 'Customer Name',field: 'company_name',width:'100%',
                  cellStyle: {
                ...
                { title: 'Purchase Price', field: 'cost_price',align: 'right', cellStyle: { textAlign: 'right', paddingRight: '20px', fontSize:cellStyle.fontSize, width : 50 }, },
                {
                  title: 'Sale Price', field: 'unit_price',align: 'right', cellStyle: { textAlign: 'right', paddingRight: '20px', fontSize:cellStyle.fontSize },headerStyle: {textAlign: 'right'},
                  render: rowData => rowData.unit_price !== null ? rowData.unit_price.toFixed(2) : ''
                },
                { title: 'Margin', field: 'Margin',  cellStyle: { textAlign: 'right', paddingRight: '20px', fontSize:cellStyle.fontSize } ,headerStyle: {textAlign: 'right'}},
               { title: 'Quantity', width :Width('quantity'),field: 'ordered_qty',
                ...
                { title: 'Tax', field: 'sale_tax_amount',
                  cellStyle: { 
                    textAlign: 'right', 
                    paddingLeft: '10px', 
                    fontSize: cellStyle.fontSize 
                ...
                { title: 'Total', field: 'total',align: 'right', cellStyle: { textAlign: 'right', paddingRight: '20px', fontSize:cellStyle.fontSize },headerStyle: {textAlign: 'right'}
              },
                { title: 'Tax category', field: 'tax_category', cellStyle: {align: 'center'} },

                { title: 'Gst Number', field: 'GST',width :Width('gst number')},
                {
                  title: 'Amount', field: 'unit_price', align: 'right', cellStyle: { textAlign: 'right', paddingRight: '20px', fontSize:cellStyle.fontSize },headerStyle: {textAlign: 'right'},
                  render: rowData => <div style={{fontSize:'12px',width : 80}}> {rowData.unit_price.toFixed(2)} </div>
                },
                ...
                //  },
                {
                  ...
                  render: (rowData) => (
                    <Chip
                      // color='success'
                      color='primary' 
                      style={{ backgroundColor: rowData.updated_color || '#87CEFA', fontSize:cellStyle.fontSize }}
                      label={rowData.updated_label}
                    />
                  ),
                },
                {
                  ...
                  render: (rowData) => (
                    <Chip
                      color='primary' 
                      style={{ backgroundColor: rowData.delivery_color || '#87CEFA', fontSize:cellStyle.fontSize }}
                      label={rowData.delivery_label}
                    />
                  ),
                },
                {
                  title: 'Payment', field: 'payment_label',cellStyle:{align: 'center'},
                  render: (rowData) => (
                    <Chip 
                      style={{ backgroundColor: rowData.payment_color || '#87CEFA', fontSize:cellStyle.fontSize }}
                      label={rowData.payment_label} 
                    />
                  ) 
                },
    ...
    // split by comma and trim spaces
                    const payments = rowData.payment_type?.split(",").map(p => p.trim()) || [];
                    // find only "Cash..." part
                    const cash = payments.find(p => p.startsWith("Cash"));
                    return cash || ""; // show only Cash part, empty if not present
                  }
                },

                {
                  title: 'Other Payment',width : 90, field: 'payment_type',
                    render: rowData => {
                      ...
                      const others = payments.filter(p => !p.startsWith("Cash"));
...
// split by comma and trim spaces
  ...
  handleShareOpen = () => this.setState({shareOpen: true});
  ...
  ApplyButton = async (formValue) => {
    ...
    console.log(this.state.from,this.state.to,'staesdddds')
    //   if(moment(this.state.from ,'year') <= moment(this.state.to,'year') ){
  ...
  render() {
    ...
    const selectedRole = this.storage?.role_name;
    const salesReportExport = UserRightsAuthorization(menuAccess[selectedRole],'reports__transactions__sales','can_export');
    console.log("filtedValue",this.state.filtedValue)
      console.log("bhgvjgfcvjfgcf",this.state.filtedValue)
    return (
      <>
       <Helmet>
                <meta charSet="utf-8" />
                <title> {titleURL} | Sales Report </title>
      </Helmet>
          ...
          >
            ...
            <MaterialTable
              ...
              components={{
                Pagination: (props) => (
                  <div
                    ...
                    <TablePagination
                      {...props}
                      count={this.props.searchSalesReportCount}
                  ...
                  icon : ()=> (
                    <div style={{display:'flex'}}>
                      ...
                      <Dialog open={this.state.shareOpen}>
                        ...
                        columns = {this.state.Schedulecolumns}
                        data = {this.props.searchSalesReportShare}
                        fromDate = {moment(this.state.from, 'year', 'month', 'day').format('yyyy-MM-DD')}
                        toDate = {moment(this.state.from, 'year', 'month', 'day').format('yyyy-MM-DD')}
                      />
                      </Dialog>
                    </div>
                  ),
              ...
              onRowsPerPageChange={(size) => this.handlePageSizeChange(size)}
                ...
                cellStyle,
                search: false,
                exportButton: salesReportExport,
                filtering: false,
                pagination: true,
                ...
                exportMenu: salesReportExport ? [
                  ...
                  //           : payment_type.map((p) => `${p.payment_type} (INR)`),
                  //     };
                  //     this.props.salesReportDataGet(
                  //       data,
                  //       setModalTypeHandler,
                  //       setLoaderStatusHandler,
                  //       (exportData) => {
                  //         const processedData = exportData.map((m) => {
                  //           const payments = m.payment_type
                  //             ? m.payment_type.split(",").map((p) => p.trim())
                  //             : [];

                  //           return {
                  //             ...m,
                  //             CashPayment: payments.find((p) => p.startsWith("Cash")) || "-",
                  //             OtherPayment: payments.filter((p) => !p.startsWith("Cash")).join(", ") || "-"
                  //           };
                  //         });

                  //         // define export columns explicitly (with Cash/Other fields)
                  //         const exportCols = [
                  //           ...this.columns.filter(c => !["payment_type"].includes(c.field)), // keep your normal columns
                  //           { title: "Cash Payment", field: "CashPayment" },
                  //           { title: "Other Payment", field: "OtherPayment" }
                  //         ];

                  //         ExportPdf(exportCols, processedData, "SalesReportData");
                  //       }
                  //     );
                  //   },
                  // },
                  {
                    ...
                    exportFunc: async (cols, datas) => {
                      ...
                      //         setLoaderStatusHandler,
                      //         (exportData) => {
                                const processedData = this.props.searchSalesReportShare.map((m) => {
                                  const payments = m.payment_type
                                    ? m.payment_type.split(",").map((p) => p.trim())
                                    : [];
                                ...
                                // define export columns explicitly (with Cash/Other fields)
                                const exportCols = [
                                  ...(this.state.tabValue === 'lotWise' ? this.columns : this.saleWiseDataColumns).filter(c => !["payment_type"].includes(c.field)), // keep your normal columns
                                  { title: "Cash Payment", field: "CashPayment" },
                                  { title: "Other Payment", field: "OtherPayment" }
                                ];

                                ExportCsv(exportCols, processedData, "SalesReportData");
                            //   }
                            // );
                    },
                  },] : [],
            ...
            // ) AS company_name,
              ...
              columns={this.state.tabValue === 'lotWise' ? this.columns : this.saleWiseDataColumns}


              title={
                <Typography variant='h6' align='left' style={{ paddingTop: '10px', paddingBottom: '10px' }}>
                  {/* <Link href='/report' underline="hover">Home</Link> / Sales Report */}
...
const mapStateToProps = state => {
  return {
    ...
    searchSalesReportShare: state.salesReducer.searchSalesReportShare || [],
