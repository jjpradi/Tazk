...
export default function SalesReturnCnNew() {
  ...
  const fetchData = async (p = 0, ps = 20) => {
    ...
    setLoading(true);
    try {
      const locPayload = headerLocationId && headerLocationId !== 'null' && headerLocationId !== '0' ? headerLocationId : 'null';
      const res = await ReportsService.getSalesReturnCn({ location_id: locPayload, pageCount: p, numPerPage: ps, fromDate, toDate });
  ...
  useEffect(() => { if (headerLocationId) fetchData(page, pageSize); }, [page, pageSize, headerLocationId, fromDate, toDate]);

  const columns = [{ field: 'invoice_number', headerName: 'CN #', flex: 0.5, minWidth: 100 }, { field: 'customer_name', headerName: 'Customer', flex: 0.8, minWidth: 120 }, { field: 'location', headerName: 'Location', flex: 0.5, minWidth: 80 }, { field: 'total', headerName: 'Amount', flex: 0.5, minWidth: 80, align: 'right', headerAlign: 'right', renderCell: (p) => p.row?.total ? `\u20B9${Number(p.row.total).toLocaleString('en-IN')}` : '-' }, { field: 'comment', headerName: 'Comment', flex: 0.5, minWidth: 80 }, { field: 'reference', headerName: 'Ref', flex: 0.4, minWidth: 70 }, { field: 'return_time', headerName: 'Date', flex: 0.5, minWidth: 90, renderCell: (p) => p.row?.return_time ? moment(p.row.return_time).format('DD MMM YY') : '-' }];

  return (
    <>
      <Helmet><title>{titleURL} | Sales Return Credit Notes</title></Helmet>
      <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1.5, minHeight: 48, flexWrap: 'wrap' }}>
          <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5 }}>Sales Return Credit Notes</Typography>
          ...
          <Tooltip title="Export CSV"><IconButton onClick={async () => {
            try { const r2 = await ReportsService.getSalesReturnCn({ location_id: 'null', pageCount: 0, numPerPage: 100000, fromDate, toDate });
              ExportCsv(columns.map(c => ({ title: c.headerName, field: c.field })), r2.data?.data || [], 'Sales_Return_Credit_Notes'); } catch(e) {}
        ...
        <Box sx={{ flex: 1, overflow: 'hidden' }}>
          <DataGrid rows={searchVal ? data.filter(row => Object.values(row).some(v => v != null && String(v).toLowerCase().includes(searchVal.toLowerCase()))) : data} columns={columns} rowCount={rowCount} getRowId={(row) => `${row.return_id}`}
