...
import { getExcessLotAction, getRelievedEmployeeDetailsAction, getSearchRelievedEmployeeDetails, setSearchRelievedEmployeeDetails } from 'redux/actions/reports_actions';
...
const MissingProducts = (props) => {
    ...
    const { reportsReducer: {excessLots} } = useSelector((state) => state);
    ...
    dispatch(getExcessLotAction(payLoad))
    )
    }, [page, pageSize])
    ...
    const finalData = excessLots?.data
    const dataWithId = finalData?.length ? finalData?.map((row, index) => ({ ...row, id: index })) : []
    console.log("dsjifgh", dataWithId, excessLots)
    return (
        <>
            <Helmet>
                <meta charSet='utf-8' />
                <title> {titleURL} | Excess lots Reports </title>
            </Helmet>
            <Box sx={{ display: 'flex', gap: 1, pb: 1, px: 2.5, pt: 1 }}>
              {[
                { label: 'Total Excess', value: excessLots?.numRows || 0, color: '#E65100' },
...
>
  ...
  <Typography className='page-title'>Excess Lots Reports</Typography>
</Breadcrumbs>
                }
                rowCount={excessLots?.numRows}
