...
const COLUMNS = [{ field: 'date', headerName: 'Date', flex: 0.3, minWidth: 100 },
  ...
  { field: 'qty_sold', headerName: 'Qty', flex: 0.3, minWidth: 55, align: 'right', headerAlign: 'right' },
...
export default function DailySalesSummaryNew() {
  ...
  const fetchData = async (p = 0, ps2 = 50) => {
    ...
    try { const res = await ReportsService.getDailySalesSummary({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: p, numPerPage: ps2, fromDate, toDate });
  ...
  return (<><Helmet><title>{titleURL} | Daily Sales Summary</title></Helmet>
    <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1, minHeight: 42, flexWrap: 'nowrap' }}>
        <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5, whiteSpace: 'nowrap' }}>Daily Sales Summary</Typography>
        <Box sx={{ flex: 1 }} />
        ...
        <Tooltip title="Export CSV"><IconButton onClick={async () => { try { const res = await ReportsService.getDailySalesSummary({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: 0, numPerPage: 10000, fromDate, toDate }); ExportCsv(COLUMNS.map(c => ({ title: c.headerName, field: c.field })), res.data?.data || [], 'Daily_Sales_Summary'); } catch(e) {} }}><FileDownloadIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
