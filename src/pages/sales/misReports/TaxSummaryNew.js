...
const COLUMNS = [{ field: 'sales_taxable', headerName: 'Sales Taxable', flex: 0.3, minWidth: 130, align: 'right', headerAlign: 'right', ...rc('sales_taxable') },
  { field: 'sales_tax_collected', headerName: 'Tax Collected (Sales)', flex: 0.3, minWidth: 150, align: 'right', headerAlign: 'right', ...rc('sales_tax_collected') },
  { field: 'purchase_taxable', headerName: 'Purchase Taxable', flex: 0.3, minWidth: 140, align: 'right', headerAlign: 'right', ...rc('purchase_taxable') },
  { field: 'purchase_tax_paid', headerName: 'Tax Paid (Purchase)', flex: 0.3, minWidth: 150, align: 'right', headerAlign: 'right', ...rc('purchase_tax_paid') },
  { field: 'net_tax_liability', headerName: 'Net Tax Liability', flex: 0.3, minWidth: 140, align: 'right', headerAlign: 'right', renderCell: (p) => { const v = p.row?.net_tax_liability; if (v == null) return '-'; const c2 = Number(v) >= 0 ? '#2E7D32' : '#C62828'; return <Typography sx={{ fontSize: 12, fontWeight: 600, color: c2 }}>{fmt(v)}</Typography>; } }];

export default function TaxSummaryNew() {
  ...
  const fetchData = async (p = 0, ps2 = 20) => {
    ...
    try { const res = await ReportsService.getTaxSummary({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: p, numPerPage: ps2, fromDate, toDate });
  ...
  return (<><Helmet><title>{titleURL} | Tax Summary (GST)</title></Helmet>
    <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1, minHeight: 42, flexWrap: 'nowrap' }}>
        <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5, whiteSpace: 'nowrap' }}>Tax Summary (GST)</Typography>
        <Box sx={{ flex: 1 }} />
        ...
        <Tooltip title="Export CSV"><IconButton onClick={async () => { try { const res = await ReportsService.getTaxSummary({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: 0, numPerPage: 10000, fromDate, toDate }); ExportCsv(COLUMNS.map(c => ({ title: c.headerName, field: c.field })), res.data?.data || [], 'Tax_Summary_(GST)'); } catch(e) {} }}><FileDownloadIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
