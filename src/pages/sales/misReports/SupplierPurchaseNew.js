...
const COLUMNS = [{ field: 'supplier_name', headerName: 'Supplier', flex: 0.3, minWidth: 150 },
  { field: 'bills', headerName: 'Bills', flex: 0.3, minWidth: 55, align: 'right', headerAlign: 'right' },
  { field: 'purchased', headerName: 'Purchased', flex: 0.3, minWidth: 100, align: 'right', headerAlign: 'right', ...rc('purchased') },
  { field: 'paid', headerName: 'Paid', flex: 0.3, minWidth: 90, align: 'right', headerAlign: 'right', ...rc('paid') },
  { field: 'outstanding', headerName: 'Outstanding', flex: 0.3, minWidth: 95, align: 'right', headerAlign: 'right', ...rc('outstanding') },
  { field: 'avg_bill', headerName: 'Avg Bill', flex: 0.3, minWidth: 85, align: 'right', headerAlign: 'right', ...rc('avg_bill') }];
...
export default function SupplierPurchaseNew() {
  ...
  const fetchData = async (p = 0, ps2 = 20) => {
    ...
    try { const res = await ReportsService.getSupplierPurchase({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: p, numPerPage: ps2, fromDate, toDate });
  ...
  return (<><Helmet><title>{titleURL} | Supplier wise Purchase</title></Helmet>
    <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1, minHeight: 42, flexWrap: 'nowrap' }}>
        <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5, whiteSpace: 'nowrap' }}>Supplier wise Purchase</Typography>
        ...
        <KpiCard label="Suppliers" value={kpis.totalSuppliers || 0} color="#2E3A59" />
        <KpiCard label="Purchased" value={fmt(kpis.totalPurchased)} color="#1565C0" />
        ...
        <Tooltip title="Export CSV"><IconButton onClick={async () => { try { const res = await ReportsService.getSupplierPurchase({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: 0, numPerPage: 10000, fromDate, toDate }); ExportCsv(COLUMNS.map(c => ({ title: c.headerName, field: c.field })), res.data?.data || [], 'Supplier_wise_Purchase'); } catch(e) {} }}><FileDownloadIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
        <Tooltip title="Close"><IconButton onClick={() => navigate('/report')}><CloseIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
      </Box>
      <Box sx={{ flex: 1, overflow: 'hidden' }}>
        <DataGrid rows={filteredData} columns={COLUMNS} rowCount={rowCount} getRowId={(row) => row.supplier_id || Math.random()}
