...
const COLUMNS = [{ field: 'category', headerName: 'Category', flex: 0.3, minWidth: 120 },
  { field: 'total_orders', headerName: 'Orders', flex: 0.3, minWidth: 60, align: 'right', headerAlign: 'right' },
  { field: 'qty_sold', headerName: 'Qty Sold', flex: 0.3, minWidth: 65, align: 'right', headerAlign: 'right' },
  { field: 'cost_value', headerName: 'Cost Value', flex: 0.3, minWidth: 95, align: 'right', headerAlign: 'right', ...rc('cost_value') },
  { field: 'revenue', headerName: 'Revenue', flex: 0.3, minWidth: 95, align: 'right', headerAlign: 'right', ...rc('revenue') },
  { field: 'gross_margin', headerName: 'Margin', flex: 0.3, minWidth: 90, align: 'right', headerAlign: 'right', renderCell: (p) => { const v = p.row?.gross_margin; if (v == null) return '-'; const c2 = Number(v) >= 0 ? '#2E7D32' : '#C62828'; return <Typography sx={{ fontSize: 12, fontWeight: 600, color: c2 }}>{fmt(v)}</Typography>; } },
  ...
  { field: 'roi_pct', headerName: 'ROI %', flex: 0.3, minWidth: 60, align: 'right', headerAlign: 'right', renderCell: (p) => p.row?.roi_pct != null ? p.row.roi_pct + '%' : '-' },
  { field: 'avg_selling_price', headerName: 'Avg Price', flex: 0.3, minWidth: 85, align: 'right', headerAlign: 'right', ...rc('avg_selling_price') },
  { field: 'avg_cost_price', headerName: 'Avg Cost', flex: 0.3, minWidth: 85, align: 'right', headerAlign: 'right', ...rc('avg_cost_price') }];

export default function CategoryMarginNew() {
  ...
  const fetchData = async (p = 0, ps2 = 20) => {
    ...
    try { const res = await ReportsService.getCategoryMargin({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: p, numPerPage: ps2, fromDate, toDate });
  ...
  return (<><Helmet><title>{titleURL} | Category wise Margin</title></Helmet>
    <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1, minHeight: 42, flexWrap: 'nowrap' }}>
        <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5, whiteSpace: 'nowrap' }}>Category wise Margin</Typography>
        ...
        <KpiCard label="Categories" value={kpis.totalCategories || 0} color="#2E3A59" />
        <KpiCard label="Revenue" value={fmt(kpis.totalRevenue)} color="#2E7D32" />
        <KpiCard label="Cost" value={fmt(kpis.totalCost)} color="#E65100" />
        <KpiCard label="Margin" value={fmt(kpis.totalMargin)} color="#1565C0" />
        ...
        <Tooltip title="Export CSV"><IconButton onClick={async () => { try { const res = await ReportsService.getCategoryMargin({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: 0, numPerPage: 10000, fromDate, toDate }); ExportCsv(COLUMNS.map(c => ({ title: c.headerName, field: c.field })), res.data?.data || [], 'Category_wise_Margin'); } catch(e) {} }}><FileDownloadIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
        <Tooltip title="Close"><IconButton onClick={() => navigate('/report')}><CloseIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
      </Box>
      <Box sx={{ flex: 1, overflow: 'hidden' }}>
        <DataGrid rows={filteredData} columns={COLUMNS} rowCount={rowCount} getRowId={(row) => row.category || Math.random()}
