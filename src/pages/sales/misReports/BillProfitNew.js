...
const COLUMNS = [{ field: 'invoice_number', headerName: 'Invoice #', flex: 0.3, minWidth: 100 },
  ...
  { field: 'customer_name', headerName: 'Customer', flex: 0.3, minWidth: 130 },
  { field: 'location_name', headerName: 'Location', flex: 0.3, minWidth: 85 },
  { field: 'buying_cost', headerName: 'Buying Cost', flex: 0.3, minWidth: 90, align: 'right', headerAlign: 'right' },
  { field: 'selling_price', headerName: 'Selling Price', flex: 0.3, minWidth: 90, align: 'right', headerAlign: 'right' },
  ...
  { field: 'profit_pct', headerName: 'Profit %', flex: 0.3, minWidth: 65, align: 'right', headerAlign: 'right', renderCell: (p) => p.row?.profit_pct != null ? p.row.profit_pct + '%' : '-' }];

export default function BillProfitNew() {
  ...
  const fetchData = async (p = 0, ps2 = 20) => {
    ...
    try { const res = await ReportsService.getBillProfit({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: p, numPerPage: ps2, fromDate, toDate });
  ...
  return (<><Helmet><title>{titleURL} | Bill Wise Profit Report</title></Helmet>
    <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1, minHeight: 42, flexWrap: 'nowrap' }}>
        <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5, whiteSpace: 'nowrap' }}>Bill Wise Profit Report</Typography>
        <Box sx={{ flex: 1 }} />
        ...
        <KpiCard label="Profit" value={fmt(kpis.totalProfit)} color="#1565C0" />
        <KpiCard label="Profit %" value={(kpis.profitPct || 0) + '%'} color="#7B1FA2" />
        ...
        <Tooltip title="Export CSV"><IconButton onClick={async () => { try { const res = await ReportsService.getBillProfit({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: 0, numPerPage: 10000, fromDate, toDate }); ExportCsv(COLUMNS.map(c => ({ title: c.headerName, field: c.field })), res.data?.data || [], 'Bill_Wise_Profit_Report'); } catch(e) {} }}><FileDownloadIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
        <Tooltip title="Close"><IconButton onClick={() => navigate('/report')}><CloseIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
      </Box>
      <Box sx={{ flex: 1, overflow: 'hidden' }}>
        <DataGrid rows={filteredData} columns={COLUMNS} rowCount={rowCount} getRowId={(row) => row.id || Math.random()}
