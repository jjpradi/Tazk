...
const COLUMNS = [{ field: 'salesman_name', headerName: 'Salesman', flex: 0.3, minWidth: 140 },
  { field: 'orders', headerName: 'Orders', flex: 0.3, minWidth: 60, align: 'right', headerAlign: 'right' },
  { field: 'qty_sold', headerName: 'Qty', flex: 0.3, minWidth: 50, align: 'right', headerAlign: 'right' },
  { field: 'revenue', headerName: 'Revenue', flex: 0.3, minWidth: 95, align: 'right', headerAlign: 'right', ...rc('revenue') },
  { field: 'cost_value', headerName: 'Cost', flex: 0.3, minWidth: 90, align: 'right', headerAlign: 'right', ...rc('cost_value') },
  { field: 'profit', headerName: 'Profit', flex: 0.3, minWidth: 85, align: 'right', headerAlign: 'right', renderCell: (p) => { const v = p.row?.profit; if (v == null) return '-'; const c2 = Number(v) >= 0 ? '#2E7D32' : '#C62828'; return <Typography sx={{ fontSize: 12, fontWeight: 600, color: c2 }}>{fmt(v)}</Typography>; } },
  ...
  { field: 'collected', headerName: 'Collected', flex: 0.3, minWidth: 90, align: 'right', headerAlign: 'right', ...rc('collected') },
  { field: 'outstanding', headerName: 'Outstanding', flex: 0.3, minWidth: 90, align: 'right', headerAlign: 'right', ...rc('outstanding') },
  { field: 'avg_ticket', headerName: 'Avg Ticket', flex: 0.3, minWidth: 85, align: 'right', headerAlign: 'right', ...rc('avg_ticket') }];

export default function SalesmanPerfNew() {
  ...
  const fetchData = async (p = 0, ps2 = 20) => {
    ...
    try { const res = await ReportsService.getSalesmanPerf({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: p, numPerPage: ps2, fromDate, toDate });
  ...
  return (<><Helmet><title>{titleURL} | Salesman Performance</title></Helmet>
    <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1, minHeight: 42, flexWrap: 'nowrap' }}>
        <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5, whiteSpace: 'nowrap' }}>Salesman Performance</Typography>
        <Box sx={{ flex: 1 }} />
        <KpiCard label="Salesmen" value={kpis.totalSalesmen || 0} color="#2E3A59" />
        <KpiCard label="Revenue" value={fmt(kpis.totalRevenue)} color="#2E7D32" />
        <KpiCard label="Orders" value={kpis.totalOrders || 0} color="#1565C0" />
        <KpiCard label="Collected" value={fmt(kpis.totalCollected)} color="#00695C" />
        ...
        <Tooltip title="Export CSV"><IconButton onClick={async () => { try { const res = await ReportsService.getSalesmanPerf({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: 0, numPerPage: 10000, fromDate, toDate }); ExportCsv(COLUMNS.map(c => ({ title: c.headerName, field: c.field })), res.data?.data || [], 'Salesman_Performance'); } catch(e) {} }}><FileDownloadIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
        <Tooltip title="Close"><IconButton onClick={() => navigate('/report')}><CloseIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
      </Box>
      <Box sx={{ flex: 1, overflow: 'hidden' }}>
        <DataGrid rows={filteredData} columns={COLUMNS} rowCount={rowCount} getRowId={(row) => row.salesman_id || Math.random()}
