...
const COLUMNS = [{ field: 'location_name', headerName: 'Location', flex: 0.3, minWidth: 130 },
  { field: 'sales_count', headerName: 'Sales', flex: 0.3, minWidth: 60, align: 'right', headerAlign: 'right' },
  { field: 'sales_revenue', headerName: 'Revenue', flex: 0.3, minWidth: 100, align: 'right', headerAlign: 'right', ...rc('sales_revenue') },
  { field: 'cost_of_goods', headerName: 'COGS', flex: 0.3, minWidth: 95, align: 'right', headerAlign: 'right', ...rc('cost_of_goods') },
...
export default function LocationPLNew() {
  ...
  const fetchData = async (p = 0, ps2 = 20) => {
    ...
    try { const res = await ReportsService.getLocationPL({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: p, numPerPage: ps2, fromDate, toDate });
  ...
  return (<><Helmet><title>{titleURL} | Location wise P&L</title></Helmet>
    <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1, minHeight: 42, flexWrap: 'nowrap' }}>
        <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5, whiteSpace: 'nowrap' }}>Location wise P&L</Typography>
        <Box sx={{ flex: 1 }} />
        <KpiCard label="Locations" value={kpis.totalLocations || 0} color="#2E3A59" />
        <KpiCard label="Total Revenue" value={fmt(kpis.totalRevenue)} color="#2E7D32" />
        ...
        <Tooltip title="Export CSV"><IconButton onClick={async () => { try { const res = await ReportsService.getLocationPL({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: 0, numPerPage: 10000, fromDate, toDate }); ExportCsv(COLUMNS.map(c => ({ title: c.headerName, field: c.field })), res.data?.data || [], 'Location_wise_P&L'); } catch(e) {} }}><FileDownloadIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
        <Tooltip title="Close"><IconButton onClick={() => navigate('/report')}><CloseIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
      </Box>
      <Box sx={{ flex: 1, overflow: 'hidden' }}>
        <DataGrid rows={filteredData} columns={COLUMNS} rowCount={rowCount} getRowId={(row) => row.location_id || Math.random()}
