...
const COLUMNS = [{ field: 'mode', headerName: 'Payment Mode', flex: 0.3, minWidth: 200 },
  { field: 'txn_count', headerName: 'Transactions', flex: 0.3, minWidth: 100, align: 'right', headerAlign: 'right' },
  { field: 'total_amount', headerName: 'Amount', flex: 0.3, minWidth: 120, align: 'right', headerAlign: 'right', ...rc('total_amount') },
  { field: 'pct', headerName: 'Share %', flex: 0.3, minWidth: 80, align: 'right', headerAlign: 'right', renderCell: (p) => p.row?.pct != null ? p.row.pct + '%' : '-' }];

export default function PaymentModeNew() {
  ...
  const fetchData = async (p = 0, ps2 = 20) => {
    ...
    try { const res = await ReportsService.getPaymentMode({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: p, numPerPage: ps2, fromDate, toDate });
  ...
  return (<><Helmet><title>{titleURL} | Payment Mode Analysis</title></Helmet>
    <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1, minHeight: 42, flexWrap: 'nowrap' }}>
        <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5, whiteSpace: 'nowrap' }}>Payment Mode Analysis</Typography>
        <Box sx={{ flex: 1 }} />
        ...
        <Tooltip title="Export CSV"><IconButton onClick={async () => { try { const res = await ReportsService.getPaymentMode({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: 0, numPerPage: 10000, fromDate, toDate }); ExportCsv(COLUMNS.map(c => ({ title: c.headerName, field: c.field })), res.data?.data || [], 'Payment_Mode_Analysis'); } catch(e) {} }}><FileDownloadIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
        <Tooltip title="Close"><IconButton onClick={() => navigate('/report')}><CloseIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
      </Box>
      <Box sx={{ flex: 1, overflow: 'hidden' }}>
        <DataGrid rows={filteredData} columns={COLUMNS} rowCount={rowCount} getRowId={(row) => row.mode || Math.random()}
