...
const COLUMNS = [{ field: 'customer_name', headerName: 'Customer', flex: 0.3, minWidth: 150 },
  ...
  { field: 'received', headerName: 'Received', flex: 0.3, minWidth: 90, align: 'right', headerAlign: 'right', ...rc('received') },
  { field: 'outstanding', headerName: 'Outstanding', flex: 0.3, minWidth: 95, align: 'right', headerAlign: 'right', ...rc('outstanding') },
  { field: 'avg_order', headerName: 'Avg Order', flex: 0.3, minWidth: 85, align: 'right', headerAlign: 'right', ...rc('avg_order') },
  { field: 'last_purchase', headerName: 'Last Purchase', flex: 0.3, minWidth: 100 }];

export default function CustomerRevenueNew() {
  ...
  const fetchData = async (p = 0, ps2 = 20) => {
    ...
    try { const res = await ReportsService.getCustomerRevenue({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: p, numPerPage: ps2, fromDate, toDate });
  ...
  return (<><Helmet><title>{titleURL} | Customer wise Revenue</title></Helmet>
    <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1, minHeight: 42, flexWrap: 'nowrap' }}>
        <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5, whiteSpace: 'nowrap' }}>Customer wise Revenue</Typography>
        ...
        <KpiCard label="Customers" value={kpis.totalCustomers || 0} color="#2E3A59" />
        ...
        <Tooltip title="Export CSV"><IconButton onClick={async () => { try { const res = await ReportsService.getCustomerRevenue({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: 0, numPerPage: 10000, fromDate, toDate }); ExportCsv(COLUMNS.map(c => ({ title: c.headerName, field: c.field })), res.data?.data || [], 'Customer_wise_Revenue'); } catch(e) {} }}><FileDownloadIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
        <Tooltip title="Close"><IconButton onClick={() => navigate('/report')}><CloseIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
      </Box>
      <Box sx={{ flex: 1, overflow: 'hidden' }}>
        <DataGrid rows={filteredData} columns={COLUMNS} rowCount={rowCount} getRowId={(row) => row.customer_id || Math.random()}
