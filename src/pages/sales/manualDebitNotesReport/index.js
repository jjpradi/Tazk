...
import { manualDnReportAction } from '../../../redux/actions/cndn_report_actions';
...
class ManualDebitNotesReport extends Component {
  ...
  loadData = () => {
    ...
    apiCalls(
      ...
      this.props.manualDnReportAction(data)
  ...
  handleExportExcel = async () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Manual Debit Notes');
      XLSX.writeFile(wb, 'Manual_Debit_Notes.xlsx');
    } catch (err) {
      console.error(err);
    }
  };
  ...
  render() {
    ...
    const { menuAccess = {} } = this.props;
		...
		const reportExport = UserRightsAuthorization(menuAccess[selectedRole], 'reports__transactions__manual_debit_notes', 'can_export');
    return (
      <>
        <Helmet>
          <title>{titleURL} | Manual Debit Notes Report</title>
        </Helmet>
        <Snackbar
...
const mapStateToProps = (state) => ({
  reportData: state.cndnReportReducer.manualDnData || [],
  reportCount: state.cndnReportReducer.manualDnCount || 0,
  grandTotal: state.cndnReportReducer.manualDnGrandTotal || 0,
  monthlySummary: state.cndnReportReducer.manualDnMonthlySummary || [],
  menuAccess: state.rbacReducer.menuAccess || [],
});

const mapDispatchToProps = (dispatch) => ({
  manualDnReportAction: (data) => dispatch(manualDnReportAction(data)),
});

export default withRouter(connect(mapStateToProps, mapDispatchToProps)(ManualDebitNotesReport));
