...
import { returnDnReportAction } from '../../../redux/actions/cndn_report_actions';
...
const DN_WISE_COLUMNS = [
  { headerName: 'DN Number', field: 'dn_number', width: 160 },
  { headerName: 'DN Date', field: 'dn_date', width: 120 },
  { headerName: 'Vendor Name', field: 'vendor_name', width: 220 },
  ...
  { headerName: 'DN Amount', field: 'dn_amount', width: 130, align: 'right' },
...
const LOT_WISE_COLUMNS = [
  { headerName: 'DN Number', field: 'dn_number', width: 160 },
  { headerName: 'DN Date', field: 'dn_date', width: 120 },
  { headerName: 'Vendor Name', field: 'vendor_name', width: 200 },
...
class ReturnDebitNotesReport extends Component {
  ...
  loadData = () => {
    ...
    apiCalls(
      ...
      this.props.returnDnReportAction(data)
  ...
  handleExportExcel = async () => {
    ...
    try {
      ...
      const columns = this.state.viewMode === 'lot' ? LOT_WISE_COLUMNS : DN_WISE_COLUMNS;
      const formatted = exportData.map(row =>
        Object.fromEntries(columns.map(col => [col.headerName, row[col.field]]))
      );
      const ws = XLSX.utils.json_to_sheet(formatted);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Return Debit Notes');
      XLSX.writeFile(wb, `Return_Debit_Notes_${this.state.viewMode === 'lot' ? 'LotWise' : 'DNWise'}.xlsx`);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  render() {
    ...
    const reportExport = UserRightsAuthorization(menuAccess[selectedRole], 'reports__transactions__purchase_return_debit_notes', 'can_export');

    return (
      <>
        <Helmet>
          <title>{titleURL} | Purchase Return Debit Notes Report</title>
        </Helmet>
        <Snackbar
                ...
                <Typography variant="h6" align="left" sx={{ pt: '10px', pb: '10px' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <Chip
                      label="DN Wise"
                      size="small"
                      color={this.state.viewMode === 'dn' ? 'primary' : 'default'}
                      variant={this.state.viewMode === 'dn' ? 'filled' : 'outlined'}
                      onClick={() => this.handleViewModeChange('dn')}
                      sx={{ ml: 1 }}
                    />
                    <Chip
                      label="Lot Wise"
...
const mapStateToProps = (state) => ({
  reportData: state.cndnReportReducer.returnDnData || [],
  reportCount: state.cndnReportReducer.returnDnCount || 0,
  grandTotal: state.cndnReportReducer.returnDnGrandTotal || 0,
  monthlySummary: state.cndnReportReducer.returnDnMonthlySummary || [],
  menuAccess: state.rbacReducer.menuAccess || []
});

const mapDispatchToProps = (dispatch) => ({
  returnDnReportAction: (data) => dispatch(returnDnReportAction(data)),
});

export default withRouter(connect(mapStateToProps, mapDispatchToProps)(ReturnDebitNotesReport));
