...
// Manual view — DN-wise summary
const DN_COLUMNS = [
  { field: 'dn_number', headerName: 'DN #', flex: 0.35, minWidth: 100 },
  { field: 'dn_type', headerName: 'Type', flex: 0.3, minWidth: 85,
    renderCell: (p) => {
      const t = p.row?.dn_type;
      const isReturn = t === 'Purchase Return';
  ...
  { field: 'dn_date', headerName: 'Date', flex: 0.3, minWidth: 85 },
  { field: 'supplier_name', headerName: 'Supplier', flex: 0.5, minWidth: 130 },
  { field: 'supplier_gstin', headerName: 'GSTIN', flex: 0.45, minWidth: 130 },
  ...
  { field: 'dn_amount', headerName: 'DN Amount', flex: 0.35, minWidth: 95, align: 'right', headerAlign: 'right', ...rupeeCell('dn_amount') },
...
// Purchase Return view — lot-wise item detail
const RETURN_COLUMNS = [
  { field: 'dn_number', headerName: 'DN #', flex: 0.3, minWidth: 90 },
  { field: 'dn_date', headerName: 'Date', flex: 0.25, minWidth: 80 },
  ...
  { field: 'discount', headerName: 'Discount', flex: 0.25, minWidth: 65, align: 'right', headerAlign: 'right' },
  ...
  { field: 'dn_gst_amount', headerName: 'DN GST', flex: 0.3, minWidth: 75, align: 'right', headerAlign: 'right', ...rupeeCell('dn_gst_amount') },
  { field: 'tds_rate', headerName: 'TDS %', flex: 0.2, minWidth: 55, align: 'right', headerAlign: 'right', renderCell: (p) => p.row?.tds_rate != null ? `${p.row.tds_rate}%` : '' },
  { field: 'tds_section', headerName: 'TDS Section', flex: 0.25, minWidth: 70 },
  { field: 'tds_amount', headerName: 'TDS Amt', flex: 0.3, minWidth: 75, align: 'right', headerAlign: 'right', ...rupeeCell('tds_amount') },
  { field: 'dn_amount', headerName: 'DN Amount', flex: 0.3, minWidth: 85, align: 'right', headerAlign: 'right', ...rupeeCell('dn_amount') },
...
export default function DebitNotesNew() {
  ...
  const fetchData = async (p = 0, ps = 20, vm = viewMode) => {
    ...
    try {
      ...
      const res = await ReportsService.getDebitNotes(payload);
  ...
  const columns = viewMode === 'return' ? RETURN_COLUMNS : DN_COLUMNS;

  const getRowId = (row) => {
    if (viewMode === 'return') return row.row_id || `${row.dn_number}_${row.product_name}_${row.lot_number}`;
    return `${row.id}`;
  };
  ...
  const handleExport = async () => {
    try {
      ...
      const r = await ReportsService.getDebitNotes(payload);
      ExportCsv(
        columns.map(c => ({ title: c.headerName, field: c.field })),
        r.data?.data || [],
        `Debit_Notes_${viewMode}`
      );
    } catch (e) {}
  };
  ...
  return (
    <>
      <Helmet><title>{titleURL} | Debit Notes</title></Helmet>
      <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1, minHeight: 42, flexWrap: 'nowrap' }}>
          <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5, whiteSpace: 'nowrap' }}>Debit Notes</Typography>
          {CHIPS.map((c) => (
