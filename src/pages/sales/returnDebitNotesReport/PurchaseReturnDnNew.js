...
export default function PurchaseReturnDnNew() {
  ...
  const fetchData = async (p = 0, ps = 20) => {
    ...
    setLoading(true);
    try {
      const res = await ReportsService.getPurchaseReturnDn({ location_id: 'null', pageCount: p, numPerPage: ps, fromDate, toDate });
  ...
  const columns = [{ field: 'invoice_number', headerName: 'DN #', flex: 0.5, minWidth: 100 }, { field: 'supplier_name', headerName: 'Supplier', flex: 0.8, minWidth: 120 }, { field: 'location', headerName: 'Location', flex: 0.5, minWidth: 80 }, { field: 'total', headerName: 'Amount', flex: 0.5, minWidth: 80, align: 'right', headerAlign: 'right', renderCell: (p) => p.row?.total ? `\u20B9${Number(p.row.total).toLocaleString('en-IN')}` : '-' }, { field: 'comment', headerName: 'Comment', flex: 0.5, minWidth: 80 }, { field: 'return_time', headerName: 'Date', flex: 0.5, minWidth: 90, renderCell: (p) => p.row?.return_time ? moment(p.row.return_time).format('DD MMM YY') : '-' }];

  return (
    <>
      <Helmet><title>{titleURL} | Purchase Return Debit Notes</title></Helmet>
      <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1.5, minHeight: 48, flexWrap: 'wrap' }}>
          <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5 }}>Purchase Return Debit Notes</Typography>
          ...
          <Tooltip title="Export CSV"><IconButton onClick={async () => {
            try { const r2 = await ReportsService.getPurchaseReturnDn({ location_id: 'null', pageCount: 0, numPerPage: 100000, fromDate, toDate });
              ExportCsv(columns.map(c => ({ title: c.headerName, field: c.field })), r2.data?.data || [], 'Purchase_Return_Debit_Notes'); } catch(e) {}
        ...
        <Box sx={{ flex: 1, overflow: 'hidden' }}>
          <DataGrid rows={data} columns={columns} rowCount={rowCount} getRowId={(row) => `${row.return_id}`}
