...
export default function PosSalesReportNew() {
  ...
  const fetchData = async (p = 0, ps = 20, vm = viewMode) => {
    ...
    try { const res = await ReportsService.getPosSalesReport({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: p, numPerPage: ps, fromDate, toDate, viewMode: vm });
  ...
  return (
    <><Helmet><title>{titleURL} | POS Sales Report</title></Helmet>
      <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1, minHeight: 42, flexWrap: 'nowrap' }}>
          <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5, whiteSpace: 'nowrap' }}>POS Sales Report</Typography>
          {CHIPS.map((c) => (
          ...
          <Tooltip title="Export CSV"><IconButton onClick={async () => { try { const res = await ReportsService.getPosSalesReport({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: 0, numPerPage: 100000, fromDate, toDate, viewMode }); ExportCsv(columns.map(c => ({ title: c.headerName, field: c.field })), res.data?.data || [], `POS_Sales_Report_${viewMode}`); } catch(e) {} }}><FileDownloadIcon sx={{ fontSize: 22 }} /></IconButton></Tooltip>
