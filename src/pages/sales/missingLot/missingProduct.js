...
import { getMissingLotAction, getRelievedEmployeeDetailsAction, getSearchRelievedEmployeeDetails, setSearchRelievedEmployeeDetails } from 'redux/actions/reports_actions';
import { Fade, IconButton, Tooltip, Grid,Button,Stack, Breadcrumbs, Link, Typography, Box } from '@mui/material';
...
const MissingProducts = (props) => {
    ...
    const { reportsReducer: {missingLots} } = useSelector((state) => state);
    ...
    dispatch(getMissingLotAction(payLoad))
    )
    }, [page, pageSize])
    ...
    const finalData = missingLots?.data
    const dataWithId = finalData?.length ? finalData?.map((row, index) => ({ ...row, id: index })) : []
    console.log("dsjifgh", dataWithId, missingLots)
    return (
        <>
            <Helmet>
                <meta charSet='utf-8' />
                <title> {titleURL} | Missing Lots Reports </title>
            </Helmet>
            <Box sx={{ display: 'flex', gap: 1, pb: 1, px: 2.5, pt: 1 }}>
              {[
                { label: 'Total Missing', value: missingLots?.numRows || 0, color: '#d32f2f' },
                { label: 'Page Items', value: dataWithId?.length || 0, color: '#2E3A59' },
              ].map(k => (
...
>
  ...
  >
    ...
    <Box sx={{ display: 'flex', color: '#0A8FDC', cursor: 'pointer', fontSize: '13px', fontWeight: 600, alignItems: 'center', '&:hover': { textDecoration: 'underline' } }} onClick={() => navigate('/report')}>
        <HomeIcon sx={{mr: 0.5}} fontSize='inherit' />Home
    </Box>
  <Typography className='page-title'>Missing Lots Reports</Typography>
</Breadcrumbs>
                }
                rowCount={missingLots?.numRows}
