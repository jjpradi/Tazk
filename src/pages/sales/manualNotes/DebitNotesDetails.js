...
import { creditNotesTimelineAction, getAllDebittNotesAction, getCreditNotesReceiptsByIdAction, ManualcreditSalesReturnAction, setCreditNotesReceiptsByIdAction } from 'redux/actions/manualNotes_actions'
import NewManualNotes from 'components/NewManualNotes'
import AlertDialog from '../../common/Dialog';
import MaterialTable, { MTableToolbar } from 'utils/SafeMaterialTable'
import { getbyidVendorAction } from 'redux/actions/vendor_actions'
...
const DebitNotesDetails = (props) => {
    ...
    const {
        manualNoteReducer: { getAllDebitNotes, manualSalesReturn, getCreditNotesReceiptsById, manualTimeline, manualNotes, recentCreditDebitNotes },
    ...
    const isActionDisabled =
        !((rowData?.invoice_number === null ||
            (rowData?.invoice_number === '' && rowData?.description !== 'Purchase Converted')) &&
    ...
    useEffect(() => {
        if (props.pageType === 'transaction') {
            if (recentCreditDebitNotes.data.length > 0) {
                const creditNotesIndex = recentCreditDebitNotes.data.findIndex((e) => e.id === props.data.id)
    ...
    }, [manualNotes, recentCreditDebitNotes, props.pageType])

    useEffect(() => {
        (async () => {
            if (index !== null && (manualNotes.length > 0 || recentCreditDebitNotes.data.length > 0) && index !== -1) {
                const creditNotesData = props.pageType === 'transaction' ? recentCreditDebitNotes.data[index] : manualNotes[index]
                await setRowData(creditNotesData)
            }
        })();
    }, [index, manualNotes, recentCreditDebitNotes, props.pageType])
    ...
    const columnsProduct = [
        ...
        { field: 'lot_number', title: 'Serial Number', render: (rowData) => rowData?.lots || '-' },
        { field: 'hsn_code', title: 'HSN Code' },
        { field: 'return_quantity', title: 'Quantity' },
        {
        ...
        },
        {
            ...
            cellStyle: { textAlign: 'right', paddingRight: '10px', fontSize: cellStyle.fontSize, width: 50 },
            headerStyle: { textAlign: 'right', paddingRight: '10px' },
            render: (rowData) => (rowData.item_unit_price * rowData.actual_quantity + (rowData.item_unit_price * rowData.actual_quantity) * 0.18).toFixed(2)
        }
    ]

    const columnManualnotes = [
        { field: 'date', title: 'Date', render: (rowData) => moment(rowData.date).format('DD/MM/YYYY') },
        { field: 'sequence_number', title: 'DN' },
    ...
    const address = [vendor_id_data.address, vendor_id_data.area, vendor_id_data.city, vendor_id_data.state, vendor_id_data.zip].filter(Boolean).join(', ')

    const dnStatus = getCreditNotesReceiptsById?.length > 0 ? getCreditNotesReceiptsById[0]?.adjusted === 1 ? 'Adjusted' : 'Partial Adjusted' : 'Created'
    const listData = props.pageType === 'transaction' ? recentCreditDebitNotes.data : manualNotes
                ...
                <Card sx={{ height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
                    ...
                    {props.pageType !== 'transaction' && (
                        <Box sx={{
                            ...
                            <Stack direction="row" alignItems="center" spacing={0.5}>
                                <OptionButton
                                    checkType='DebitNotes'
                                    handleDebitNotesOptionsChange={handleDebitNotesOptionsChange}
                                ...
                                <Tooltip title='Next'>
                                    <span>
                                        <IconButton size="small" onClick={handleNext} disabled={listData?.length === index + 1}>
                    ...
                    <Box sx={{ flex: 1, overflow: 'auto', p: 2.5 }}>
                        ...
                        <Grid container spacing={2} sx={{ mb: 2 }}>
                            <Grid size={{ xs: 6, sm: 3 }}>
                                <StatCard label="Debit Type" value={rowData.returnFrom === null ? 'Manual' : 'Purchase Return'} color={theme.palette.primary.main} />
                            </Grid>
                            <Grid size={{ xs: 6, sm: 3 }}>
                            ...
                            <Grid size={{ xs: 6, sm: 3 }}>
                                <StatCard label="Status" value={dnStatus} color={theme.palette.success.main} />
                            </Grid>
                        </Grid>
                                ...
                                components={{
                                    Toolbar: (props) => (
                                        <div style={{ display: 'flex', width: '100%', padding: '20px 20px 10px 20px' }}>
                                            <div>
                                                <Typography sx={{ fontSize: 12 }}>Party Name: {vendor_id_data?.company_name || '-'}</Typography>
                                            </div>
