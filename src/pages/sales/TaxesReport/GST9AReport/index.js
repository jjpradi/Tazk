...
import { getGST9AReportAction } from "redux/actions/tax_actions";
import GSTR9AFilter from "./GSTR9AFilter";
...
function GST9AReport(){
  const dispatch = useDispatch()
  const navigate = useNavigate()
  const { taxReducer: { gst9aReport }, salesReducer: { getbillingcompanydetails } } = useSelector(state => state)
  ...
  useEffect(() => {
    dispatch(getGST9AReportAction({ fromDate: from, toDate: to, sub_company_id: subcompanyId }, setModalTypeHandler, setLoaderStatusHandler))
  }, [])
  ...
  const clearButton = async () => {
    ...
    await dispatch(getGST9AReportAction({ fromDate: fiscalStart, toDate: fiscalEnd, sub_company_id: 'All' }, setModalTypeHandler, setLoaderStatusHandler))
    setFilterOpen(false)
  }

  const ApplyButton = async () => {
    await dispatch(getGST9AReportAction({ fromDate: from, toDate: to, sub_company_id: subcompanyId }, setModalTypeHandler, setLoaderStatusHandler))
    setFilterOpen(false)
  }
  ...
  return(
    <>
      <Helmet><meta charSet="utf-8" /><title>{titleURL} | GSTR 9A</title></Helmet>
      <Box p={2}>
        <Grid container display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          ...
          <div style={{display: 'flex', alignItems: 'center'}}>
            <GSTR9AFilter fromTo={true} from={from} to={to} subcompanyId={subcompanyId} dateRange={dateRange} handleChange={handleChange} handleClose={() => setFilterOpen(false)} handleOpen={() => setFilterOpen(true)} open={filterOpen} ApplyButton={ApplyButton} clearButton={clearButton} />
          </div>
        </Grid>

        <Card>
          <CardContent>
            <Typography variant="subtitle1" fontWeight="bold" mb={1}>Composition Scheme - Annual Summary</Typography>
