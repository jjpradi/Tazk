...
export default function ManualCnNew() {
  ...
  const fetchData = async (p = 0, ps = 20) => {
    setLoading(true);
    try {
      const res = await ReportsService.getManualCn({ pageCount: p, numPerPage: ps, fromDate, toDate });
    ...
    <>
      ...
      <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1.5, minHeight: 48, flexWrap: 'wrap' }}>
          <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5 }}>Manual Credit Notes</Typography>
          ...
          <Tooltip title="Export CSV"><IconButton onClick={async () => {
            try { const r2 = await ReportsService.getManualCn({ pageCount: 0, numPerPage: 100000, fromDate, toDate });
              ExportCsv(columns.map(c => ({ title: c.headerName, field: c.field })), r2.data?.data || [], 'Manual_Credit_Notes'); } catch(e) {}
