...
const NonReconciled = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await customFetch(API_URLS.LOTITEMS_RECONCILE_MISSING_STOCKS(headerLocationId), 'GET');
        setData(response?.data?.mergedArray || []);
      } catch (error) { setData([]); }
      setLoading(false);
    })();
  }, [headerLocationId]);
  ...
  return (
    <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1.5, minHeight: 48 }}>
        <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59' }}>Non-Reconciled Stocks</Typography>
        <Box sx={{ flex: 1 }} />
        <Box sx={{ px: 1.5, py: 0.5, borderRadius: 1.5, bgcolor: '#F8FAFC', border: '1px solid #E8EDF5', textAlign: 'center', minWidth: 80 }}>
          <Typography sx={{ fontSize: 9, color: '#8C8C8C', fontWeight: 500, lineHeight: 1.2 }}>Total Missing</Typography>
          <Typography sx={{ fontSize: 13, fontWeight: 700, color: '#d32f2f', lineHeight: 1.3 }}>{data.length}</Typography>
        </Box>
