...
const COLUMNS = [
  { field: 'bill_number', headerName: 'Bill #', flex: 0.4, minWidth: 110 },
  ...
  { field: 'paid_amount', headerName: 'Paid', flex: 0.35, minWidth: 85, align: 'right', headerAlign: 'right', ...rc('paid_amount') },
...
export default function PayableNew() {
  ...
  const fetchData = async (p = 0, ps = 20, af = ageFilter) => {
    ...
    try {
      ...
      const res = await ReportsService.getPayable(payload);
  ...
  const handleExport = async () => {
    try { const res = await ReportsService.getPayable({ location_id: locationFilter.length ? locationFilter.map(l => l.location_id) : 'null', pageCount: 0, numPerPage: 100000 });
      ExportCsv(COLUMNS.map(c => ({ title: c.headerName, field: c.field })), res.data?.data || [], 'Payable_Report'); } catch (e) {}
  };
  ...
  return (
    <>
      <Helmet><title>{titleURL} | Payable Report</title></Helmet>
      <Card sx={{ p: 2, width: '100%', height: 'calc(100vh - 90px)', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1, minHeight: 42, flexWrap: 'nowrap' }}>
          <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#2E3A59', mr: 0.5, whiteSpace: 'nowrap' }}>Payable Report</Typography>
          {CHIPS.map((c) => (
          ...
          <Box sx={{ flex: 1 }} />
          <KpiCard label="Bills" value={kpis.totalCount || 0} color="#2E3A59" />
