...
import solution from '../../../assets/dashboardIcons/FixedIssue.png';
...
function EmployeeCard(props) {
    ...
    const {
        PayrolldashboardReducers: { employeeCount },ErrorDashboardReducer: {error_dashboard_list, error_dashboard_list_count, error_dashboard_list_fixedCount}
  ...
  function countFixed() {
    const data22 = error_dashboard_list.filter((d) => d.current_status == "DONE");
    return data22.length;
  }

  console.log(error_dashboard_list_fixedCount, "error_dashboard_list_fixedCount")
                ...
                <Grid container>
                    <Grid
                        ...
                        <Grid container display='flex' >
                            <Grid paddingLeft={3}>
                                <img src={solution} height={50} width={50} />
                            </Grid>
                            <Grid padding='15px 0px 0px 15px'>
                                <Typography style={{ fontSize: "18px", fontWeight: 500 }}>
                                    {error_dashboard_list_fixedCount}
                                </Typography>
                                <Typography color='textSecondary'>{'Fixed Issues'}</Typography>
                            </Grid>
                        </Grid>
                    </Grid>
                    <Grid
