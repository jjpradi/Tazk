...
import apiCalls from 'utils/apiCalls';
  ...
  const LeadsTasks = () => {
    ...
    const {
      CalenderReducers: {getReminders,calenderLeadsTasks},
    ...
    useEffect(() => {
      ...
      dispatch(calenderLeadsTasksAction());
    ...
    useEffect(() => {
      ...
      const leadTasks = calenderLeadsTasks
     
      leadTasks.forEach((field) => {
        eventsObj.push({
            date : field.due_date,
            title : `Due Date - ${field.task_owner}`,
            color : '#9DBDFF',
            description : field.description,
            time : moment(field.due_date,'YYYY-MM-DD').format('DD/MM/YYYY'),
            name : field.name,
            id : field.lead_id
        })
    })
