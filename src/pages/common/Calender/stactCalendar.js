...
import { getPlanSubForClientAction, getEventsAction, ListPayrollCalender } from 'redux/actions/calender_actions'
...
const StactCalendar = () => {
 const dispatch = useDispatch()
    ...
    const {
        CalenderReducers : { getPlanSubForClient }
    ...
    useEffect(() => {
        ...
        dispatch(getPlanSubForClientAction())
    }, [])

    useEffect(() => {
        const eventsObj = []
            getPlanSubForClient.forEach((field) => {
                const employeeName = field.employee_names || 'Unknown';
                if (field.start_date) {
                    eventsObj.push({
                        date: field.start_date,
                        title: `Plan Starts - ${field.start_date} - ${employeeName}`,
                        color: '#9DBDFF',
                    });
                }
                if (field.end_date) {
                    eventsObj.push({
                        date: field.end_date,
                        title: `Plan Ends - ${field.end_date} - ${employeeName}`,
                        color: '#FFB6B6',
                    });
                }
            })
        
        setPlansList(eventsObj)
    }, [])

    const handleClickPlansOpen = (events) => {
        setSelectedPlans(events)
        setPlansOpen(true)
    }

    const handleClickPlansClose = () => {
        setPlansOpen(false)
    }
          ...
          <Grid container spacing = {2}>
              ...
              <Grid
                  ...
                  <Tooltip 
                      ...
                      placement = 'top'
                  >
                      <Button sx={{ fontSize : '30px', color : 'grey' }}
          ...
          <CalenderContainer>
              ...
              
              {
                  daysToRender.map((day, index) => {
                      ...
                      const dayEvents = plansList.filter((event) => {
                              ...
                              </Typography>
                              {
                                  dayEvents.slice(0, maxEventsToShow).map((event, index) => (
                                      <EventLabel
                                          key = {index}
                                          color = {event.color}
                                          onClick = {() => handleClickPlansOpen([event])}
                                          sx={{ 
                                              whiteSpace: 'normal',  
                                              wordWrap: 'break-word', 
                                              overflow: 'visible', 
                              ...
                              }
                              {
                                  dayEvents.length > maxEventsToShow && (
                                      <Button
                                          ...
                                          onClick = {() => handleClickPlansOpen(dayEvents)}
                                      >
