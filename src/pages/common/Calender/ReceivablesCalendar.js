...
import apiCalls from 'utils/apiCalls';
  ...
  const ReceivablesCalender = () => {
    ...
    useEffect(() => {
      ...
      const receivable = getReminders.filter((e)=> e.type === 'receivable')
      console.log(getReminders,'getReminders4444')
      receivable.forEach((field) => {
        eventsObj.push({
            date : field.date,
            title : ` ${field.title}`,
            color : '#9DBDFF',
            ...
            time : field.time,
            so_number : field.invoice_number,
    ...
    console.log(selectedDaysEvents,'selectedDaysEvents222',data)
      ...
      <Box sx={{overflow: 'auto'}}>
        ...
        <CalenderContainer>
          ...
          {daysToRender.map((day, index) => {
            ...
            console.log(sale_outstanding,'sale_outstanding555')
                    ...
                    >
                      ...
                      >
                        <Typography variant='body1'>
                          {`${event.title}-${event.time} - ${event.so_number}`}
                        </Typography>
        ...
        <Dialog open={edit}>
          <ReminderForm handleClose={() => setEdit(false)} datas = {sale_outstanding} data={data} type = {'receivable'}/>
        </Dialog>
      </Box>
    );
  };
  
  export default ReceivablesCalender;
