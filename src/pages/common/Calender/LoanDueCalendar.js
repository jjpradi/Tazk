import {
  ...
  import {getLoanDuesAction} from 'redux/actions/calender_actions';
...
import apiCalls from 'utils/apiCalls';
  ...
  const LoanDueCalendar = () => {
    ...
    useEffect(() => {
      ...
      dispatch(getLoanDuesAction(payload));
    ...
    useEffect(() => {
      ...
      getLoanDues.forEach((field) => {
        const dueDate = dayjs(field.reminder_date).year(currentYear)
        
        eventsObj.push({
            date : dueDate.format('YYYY-MM-DD'),
            title : `${field.loan_number} - ${field.emp_name}`,
            color : '#9DBDFF',
  ...
  export default LoanDueCalendar;
