...
import { listPayablesAction } from 'redux/actions/purchase_actions';
import apiCalls from 'utils/apiCalls';
  ...
  const PayableCalender = () => {
    ...
    useEffect(() => {
      ...
      const payable = getReminders.filter((e)=> e.type === 'payable')

      console.log(payable,'payabaleee')
     
      payable.forEach((field) => {
        eventsObj.push({
            date : field.date,
            title : `${field.title}`,
            color : '#9DBDFF',
    ...
    return (
      <Box sx={{overflow: 'auto'}}>
                    ...
                    >
                      ...
                      >
                        <Typography variant='body1'>
                          {`${event.title}-${event.time}-${event.po_number}`}
                        </Typography>
        ...
        <Dialog open={edit}>
          <ReminderForm handleClose={() => setEdit(false)} datas = {purchase_outstanding} data={data} type = {'payable'}/>
        </Dialog>
      </Box>
    );
  };
  
  export default PayableCalender;
