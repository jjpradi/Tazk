...
const ReportSchedule = () => {
  ...
  const {
    CalenderReducers: {getReportSchedule},
  ...
  useEffect(() => {
    ...
    dispatch(calenderReportScheduleAction());
  }, []);

  useEffect(() => {
    const eventsObj = [];
    const Reminders = getReportSchedule;
    Reminders.forEach((field) => {
      eventsObj.push({
        date: field.date_time,
        title: field.report_name,
        color: '#9DBDFF',
        name: field.export_type,
        time: field.date_time,
        description: field.frequency,
        id: field.id,
        email : field.email,
        whatsApp : field.whatsApp,
        email_receipient : JSON.parse(field.email_recipient),
  ...
  <Stack spacing={2} divider={<Divider flexItem />}>
    ...
    <Box>
      ...
      <Typography variant="body1">
        {`${event.description} (${moment(event.time, 'YYYY-MM-DD HH:mm:ss').format('hh:mm A')}) - ${event.name}`}
      </Typography>
    </Box>
    ...
    <Box>
      ...
      <Stack spacing={0.5}>
        {event.email_receipient?.map((e, index) => (
