...
import { deleteDesignationAction, deletePlanTypeAction, designationAction, getSearchDesignation, planTypeAction, setSearchDesignation, setSearchPlanType } from 'redux/actions/userCreation_actions';
...
export default function PlanType() {
    ...
    const { setModalTypeHandler, setLoaderStatusHandler, commoncookie, headerLocationId } = useContext(
      ...
      const { UserRoleReducer: { planType} } = useSelector((state) => state);

    useEffect(() => { (async () => {
            dispatch(
                planTypeAction(async (response) => {
    ...
    const handleDelete = async()=>{
        setOpen(false);
        await dispatch(deletePlanTypeAction(rowData.id));
    }
  ...
  return (
      <>
          ...
          <Dialog open={designationlist === true}>
                  <PlanTypeForm handleClose={handleClose} setSearch ={setSearch}/>
                  </Dialog>
