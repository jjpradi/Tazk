...
import total from '../../../assets/dashboardIcons/totalAssets.jpg';
...
import { getAuditDash, getTtotalAssetsAction } from 'redux/actions/asset_actions';
import CurrencyRupeeIcon from '@mui/icons-material/CurrencyRupee';
import DashboardTile from 'components/DashboardTile';
class UnAssignedCard extends Component {
    ...
    async componentDidMount() {
        ...
        //     await  this.props.getTtotalAssetsAction(context.headerLocationId);
        // }
    ...
    //     if (preProps.getTotalAssets !== this.props.getTotalAssets) {
    //         const { count, status } = this.props.getTotalAssets;
    //         this.setState({
    //             chartData: {
    //                 count: count || 0,
    //                 status: status || 0,
    ...
    //           context.setModalTypeHandler,
    //           context.setLoaderStatusHandler,
    //           this.props.getTtotalAssetsAction(context.headerLocationId)
    //         );
    //       }
...
console.log(this.props.data,"dasdsad");
        ...
        return (
          ...
          >
            <DashboardTile
              ...
              icon={total}
              value={this.props?.data[0]?.totalAssets || 0}
              currencyIcon={false}
            />
          </div>
        );
...
const mapStateToProps = (state) => {
    return {
        getTotalAssets: state.AssetReducers.getTotalAssets || [],
...
const mapDispatchToProps = (dispatch) => {
    ...
    return {
        getTtotalAssetsAction: () => {
            return dispatch(getTtotalAssetsAction());
        },
...
export default useCommonRef(connect(mapStateToProps, mapDispatchToProps)(UnAssignedCard));		