...
import audit from '../../../assets/dashboardIcons/audit.png';
...
import { getAuditDash, getUnAuditedDataAction } from 'redux/actions/asset_actions';
import CurrencyRupeeIcon from '@mui/icons-material/CurrencyRupee';
import DashboardTile from 'components/DashboardTile';
class UnAuditedCard extends Component {
    ...
    //         await  this.props.getUnAuditedDataAction(context.headerLocationId);
    //     }
    ...
    //     if (preProps.getUnAudited !== this.props.getUnAudited) {
    //         const { count, status } = this.props.getUnAudited;
    //         this.setState({
    //             chartData: {
    //                 count: count || 0,
    //                 status: status || 0,
    ...
    //           context.setModalTypeHandler,
    //           context.setLoaderStatusHandler,
    //           this.props.getUnAuditedDataAction(context.headerLocationId)
    //         );
    //       }
        ...
        return (
          ...
          >
            <DashboardTile
              ...
              title='Unaudited'
              icon={audit}
              value={this.props?.data[0]?.unAudited || 0}
              currencyIcon={false}
            />
          </div>
        );
...
const mapStateToProps = (state) => {
    return {
        getUnAudited: state.AssetReducers.getUnAudited || [],
...
const mapDispatchToProps = (dispatch) => {
    ...
    return {
        getUnAuditedDataAction: () => {
            return dispatch(getUnAuditedDataAction());
        },
...
export default useCommonRef(connect(mapStateToProps, mapDispatchToProps)(UnAuditedCard));		