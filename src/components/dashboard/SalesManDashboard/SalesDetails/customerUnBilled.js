...
const CustomerUnBilled = (props) => {
    ...
    return (
        ...
        >
            <DashboardTile
                {...props}
                title='No.of Customers UnBilled'
                icon={custIcon}
                value={props?.data[0]?.UnbilledCount?.toLocaleString() || '0'}
                currencyIcon={false}
            />
        </div>
    );
...
export default useCommonRef(CustomerUnBilled);
