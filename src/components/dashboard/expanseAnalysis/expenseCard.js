...
import { Grid, CardContent, Typography, Card, ThemeProvider, useMediaQuery, IconButton, Stack, Box } from '@mui/material';
...
import { currentMonthExpenseAction, listExpenseChart } from 'redux/actions/profitloss_actions';
...
class ExpenseCard extends Component {
  ...
  //           await this.props.currentMonthExpenseAction(context.headerLocationId);
  //       }
  //     // clientwebsocket.socket.onmessage = async (message) => {
  //     //       let { event } = JSON.parse(message.data)
  //     //       if (event === 'ExpensesAnalysis') {
  //     //           await this.props.currentMonthExpenseAction();
  //     //       }
  //     //   }
  //   }
  ...
  //           this.props.currentMonthExpenseAction(context.headerLocationId)
  //         );
  //       }
    
  //       if (preProps.current_month_expense !== this.props.current_month_expense) {
  //           for (let i of this.props.current_month_expense) {
  ...
  // pollData = () => {
  //   const context = this.context;
  //     this.props.pollServer(this.props.currentMonthExpenseAction(context.headerLocationId))
  // }
  render() {
    ...
    return (
      ...
      >
        <DashboardTile
            ...
            value={this.props?.data[0]?.monthtotal.toFixed(2) || '0.00'}
            currencyIcon={true}
            secondaryText='from last month'
            arrow={true}
            arrowData={{PreMonthTotal: this.props?.data[0]?.PreMonthTotal || '0.00', monthtotal: this.props?.data[0]?.monthtotal || '0.00'}}
        />
      </div>
    );
}
...
const mapStateToProps = (state) => {
    return {
        ...
        current_month_expense:state.profitlossReducer.current_month_expense || []
    };
};
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        currentMonthExpenseAction: (data) => {
          return dispatch(currentMonthExpenseAction(data));
      },
...
export default useCommonRef(connect(mapStateToProps, mapDispatchToProps)(ExpenseCard));
