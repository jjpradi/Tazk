...
const SaleCard = (props) => {
    ...
    //             .get(
    //                 `dashboard/widgets/sale_price/${headerLocationId}`,
    //             )
...
// const finalData = isNaN(widge[0]?.sale_price) || widge[0]?.sale_price === null ? 0 : widge[0]?.sale_price
    ...
    return (
        ...
        >
            <DashboardTile
                ...
                title='Sale'
                icon={saleIcon}
                value={props?.data[0]?.sale_price || '0.00'}
                currencyIcon={true}
            />
        </div>
    );
...
export default useCommonRef(SaleCard);
