...
const WidgetGrossProfit = (props) => {
    ...
    //                   .get(
    //                       `dashboard/grossProfits/${headerLocationId}`,
    //                   )
    ...
    // const GrossProfit = () => {
    //     console.log('grosssprofit', profit[0])
    //     let gross = profit[0]?.grossprofit
    //     return gross;
    // }

    // const finalData  = (isNaN(profit[0]?.grossprofit) || profit[0]?.grossprofit === null  )? 0 : (profit[0]?.grossprofit).toFixed(2)
    return (
        <div
            ref={(el) => {
                props.ref1(el)
        ...
        >
            <DashboardTile
                ...
                title='Gross Profit'
                icon={growthIcon}
                value={props?.data[0]?.grossprofit.toFixed(2) || '0.00'}
                currencyIcon={true}
            />
        </div>
    );
...
export default useCommonRef(WidgetGrossProfit);
