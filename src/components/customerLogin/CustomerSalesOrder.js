...
const CustomerSalesOrder = (props) => {
  const dispatch = useDispatch();

  const [customerIndex, setCustomerIndex] = useState(index);
  ...
  const {
    customerReducer: {customerInvoice},
  ...
  useEffect(() => {
    ...
    dispatch(customerInvoiceAction(payload));
  ...
  const handleSubmit = async () => {
    ...
    await dispatch(customerInvoiceAction(payload));
    setFilter(false);
  };
    ...
    <>
      <Card>
        <MaterialTable
          ...
          title={'Sale Orders'}
          totalCount={customerInvoice.numRows}
          options={{
